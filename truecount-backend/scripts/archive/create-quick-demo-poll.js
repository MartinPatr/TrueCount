...
async function createQuickDemoPoll() {
  try {
    console.log('🏛️ Creating Quick Student Government Demo Poll...');
    
    // Create a poll with 5 seconds commit phase for immediate demo
    const commitSeconds = 5; // 5 seconds
    ...
    console.log('\n⏳ Waiting for commit phase to end (5 seconds)...');
    await new Promise(resolve => setTimeout(resolve, 6000)); // Wait 6 seconds to be sure
    
    // Check if we're in reveal phase now
    ...
    if (phase === 'REVEAL') {
      ...
      console.log('\n🎉 Ready for reveal demonstration!');
      ...
      console.log('❌ Poll is still in COMMIT phase. Time may not have advanced properly.');
    }
    
  } catch (error) {
    console.error('❌ Quick demo poll creation failed:', error);
  }
}
