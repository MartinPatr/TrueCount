...
async function voteInPoll1() {
  try {
    console.log('🏛️ Voting in Poll 1 (Already in Reveal Phase)');
    ...
    if (phase !== 'REVEAL') {
      console.log('❌ Poll is not in reveal phase. Advancing time...');
    ...
    // Check if we can still commit (we shouldn't be able to in reveal phase)
    ...
    if (!hasCommitted) {
      ...
      if (finalPhase === 'REVEAL') {
        ...
        console.log('💡 You can still use this poll, but you\'ll need to wait for the commit phase to end naturally.');
      }
